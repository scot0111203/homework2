...
```{r ans12}
names(studentRecord[[4]])[c(1:4)] <- c("一","二","三","四")
studentRecord
```
...
```{r ans21}
commitCheck[[8]][["commit"]][["committer"]][["date"]] -> commit8Date
commit8Date
```
...
```{r ans23}
length(commitCheck)
names(commitCheck)[c(1:30)] <- c("commit1","commit2","commit3","commit4","commit5","commit6","commit7","commit8","commit9","commit10","commit11","commit12","commit13","commit14","commit15","commit16","commit17","commit18","commit19","commit20","commit21","commit22","commit23","commit24","commit25","commit26","commit27","commit28","commit29","commit30")
commitCheck
```
...
```{r ans26}
hour(allCommitDatesTW)
minute(allCommitDatesTW)
logicWithinTimeFrame  <-     hour(allCommitDatesTW) == 4 
...
```{r ans32}
logicFromEcon <- transcriptData[["學系"]] == "經濟系"
logicFromEcon
```
...
```{r ans33}
transcriptData [c("學號","學分數","必選修類別（必∕選∕通）")] ->transcriptPartialData
...
transcriptPartialDataEcon <- list(
  學號 = transcriptPartialData[["學號"]] 
  學分數 = transcriptPartialData[["學分數"]] 
  必選修類別（必∕選∕通）=transcriptPartialData [["必選修類別（必∕選∕通）"]] 
